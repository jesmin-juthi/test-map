...
function wpestate_myposition(map) {
	...
	if (navigator.geolocation) {
		...
		if (location.protocol === 'https:') {
			navigator.geolocation.getCurrentPosition(wpestate_showMyPosition_original, wpestate_errorCallback, {
				timeout: 10000
			});
		} else {
			...
			if (typeof map != 'undefined') {
				...
				markers_cluster = L.markerClusterGroup({
					...
					iconCreateFunction: function (cluster) {
						return L.divIcon({
							html: '<div class="leaflet_cluster">' + cluster.getChildCount() + '</div>'
						});
					},
				});
...
function wpestate_enable_half_map_pin_action() {
	...
	jQuery('#google_map_prop_list_sidebar .listing_wrapper').on({
		mouseenter: function () {
			...
			// Pin on hover the saved
			if (Object.values(ajaxcalls_vars.__ok_pinned_property_id).indexOf(listing_id) > -1) {
				__wpestate_marked_custom_pin(listing_id)
			} else {
